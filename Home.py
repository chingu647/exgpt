...
# ###########################################################
from src import admin_sidebar as as

# 1. 공통 UI 컴포넌트 로드 (사이드바 메뉴 상시 노출)
as.render_admin_sidebar()
...
if current == "휴게소 챗봇":
    from views import service_chatbot as sc
    sc.show_chatbot() 

elif current == "휴게소 성과":
    from views import service_overview as so
    so.show_overview() 

elif current == "입점업체 지원":
    from views import seller_support as ss
    ss.show_support() 
